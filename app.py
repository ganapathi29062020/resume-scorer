...
{{
  ...
  "project_fit": 0,
  "learning_resources": [
    {{"skill": "Docker", "resource_type": "YouTube", "link": "https://youtube.com/example"}}
  ]
}}
...
- missing_skills must contain only real missing skills.
...
"""
        ...
        try:
            ...
            for item in result.get("learning_resources", []):
                st.write(f"- **{item.get('skill','')}** ({item.get('resource_type','')}): {item.get('link','')}")
